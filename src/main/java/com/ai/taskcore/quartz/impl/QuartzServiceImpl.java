package com.ai.taskcore.quartz.impl;

import java.util.HashMap;
import java.util.List;

import org.quartz.CronTrigger;
import org.quartz.JobDetail;
import org.quartz.Trigger;
import org.quartz.impl.StdScheduler;

import com.ai.frame.bean.InputObject;
import com.ai.frame.bean.OutputObject;
import com.ai.taskcore.quartz.IQuartzManager;
import com.ai.taskcore.quartz.IQuartzService;

public class QuartzServiceImpl implements IQuartzService{
	// Quartz调度器对象
	private StdScheduler scheduler;
	// 调度管理类
	private IQuartzManager quartzManager;

	public void setScheduler(StdScheduler scheduler) {
		this.scheduler = scheduler;
	}

	public void setQuartzManager(IQuartzManager quartzManager) {
		this.quartzManager = quartzManager;
	}

	/**
	 * 修改job描述信息
	 * 
	 * @param triggerName
	 * @param triggerGroup
	 * @param trigger
	 * @return
	 */
	public void updateJobDesc(InputObject inputObject, OutputObject outputObject) throws Exception{
		Trigger trigger = null;
		String triggerName=inputObject.getParams().get("triggerName");
		String triggerGroup=inputObject.getParams().get("triggerGroup");
		String description=inputObject.getParams().get("description");
		trigger = scheduler.getTrigger(triggerName, triggerGroup);
		trigger.setDescription(description);
		quartzManager.updateTrigger(triggerName, triggerGroup, trigger);
	}

	/**
	 * 更改执行计划
	 * 
	 * @param triggerName
	 * @param triggerGroup
	 * @param cronTrigger
	 * @return
	 */
	public void updateCronExpression(InputObject inputObject, OutputObject outputObject) throws Exception{
		String triggerName=inputObject.getParams().get("triggerName");
		String triggerGroup=inputObject.getParams().get("triggerGroup");
		String cronExpression=inputObject.getParams().get("cronExpression");
		CronTrigger cronTrigger = (CronTrigger) scheduler.getTrigger(triggerName, triggerGroup);
		cronTrigger.setCronExpression(cronExpression);
		quartzManager.updateCronTrigger(triggerName, triggerGroup, cronTrigger);
	}

	/**
	 * 修改job名称
	 * 
	 * @param jobDetail
	 * @return
	 */
	public void updateJobName(InputObject inputObject, OutputObject outputObject) throws Exception{
		String jobName=inputObject.getParams().get("jobName");
		String jobGroup=inputObject.getParams().get("jobGroup");
		String newJobName=inputObject.getParams().get("newJobName");
		JobDetail jobDetail = scheduler.getJobDetail(jobName, jobGroup);
		jobDetail.setName(newJobName);
		quartzManager.updateJobDetail(jobDetail);
	}

	/**
	 * 删除Job,直接从数据库中删除，关联表数据也删除
	 * 
	 * @param triggerName
	 * @param triggerGroup
	 * @param jobName
	 * @param jobGroup
	 */
	public void deleteJob(InputObject inputObject, OutputObject outputObject) throws Exception{
		String triggerName=inputObject.getParams().get("triggerName");
		String triggerGroup=inputObject.getParams().get("triggerGroup");
		String jobName=inputObject.getParams().get("jobName");
		String jobGroup=inputObject.getParams().get("jobGroup");
		quartzManager.deleteJob(triggerName, triggerGroup, jobName, jobGroup);
	}

	/**
	 * 暂停某个触发器
	 * 
	 * @param triggerName
	 * @param triggerGroup
	 * @return
	 */
	public void pauseTrigger(InputObject inputObject, OutputObject outputObject) throws Exception{
		String triggerName=inputObject.getParams().get("triggerName");
		String triggerGroup=inputObject.getParams().get("triggerGroup");
		quartzManager.pauseTrigger(triggerName, triggerGroup);
	}

	/**
	 * 恢复已暂停的触发器
	 * 
	 * @param triggerName
	 * @param triggerGroup
	 * @return
	 */
	public void resumeTrigger(InputObject inputObject, OutputObject outputObject) throws Exception{
		String triggerName=inputObject.getParams().get("triggerName");
		String triggerGroup=inputObject.getParams().get("triggerGroup");
		quartzManager.resumeTrigger(triggerName, triggerGroup);
	}

	/**
	 * 立即执行JOB
	 * 
	 * @param jobName
	 * @param jobGroup
	 * @return
	 */
	public void runAJobNow(InputObject inputObject, OutputObject outputObject) throws Exception{
		String jobName=inputObject.getParams().get("jobName");
		String jobGroup=inputObject.getParams().get("jobGroup");
		scheduler.triggerJob(jobName, jobGroup);
	}
	/**
	 * 暂停所有触发器
	 * 
	 * @return
	 */
	public void pauseAllTrigger(InputObject inputObject, OutputObject outputObject) throws Exception{
		quartzManager.pauseAllTrigger();
	}

	/**
	 * 启动所有触发器
	 * 
	 * @return
	 */
	public void startAllTrigger(InputObject inputObject, OutputObject outputObject) throws Exception{
		quartzManager.startAllTrigger();
	}

	/**
	 * 停止所有触发器,停止后无法启动，无法调用start方法
	 * 
	 * @return
	 */
	public void shutdownTrigger(InputObject inputObject, OutputObject outputObject) throws Exception{
		quartzManager.shutdownTrigger();
	}

	/**
	 * 返回所有jobDetail
	 * 
	 * @return
	 */
	public void getAllJobDetail(InputObject inputObject, OutputObject outputObject) throws Exception{
		List<HashMap<String, String>> list = quartzManager.getAllJobDetail();
		outputObject.getBeans().addAll(list);
	}
}
